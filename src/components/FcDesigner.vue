...
<template>
            ...
            <div class="_fc-m-tools" height="45">
                <a-button type="primary" plain round size="small"
                        ...
                        <a-popconfirm
                            ...
                            <a-button type="danger" plain round size="small"> <i class="fc-icon icon-delete"></i>清 空
                                </a-button>
    ...
    <a-layout-sider
      ...
      >
      <a-tabs>
          <a-tab-pane key="1" name="ctl" tab="组件配置">
            <a-divider v-show="showBaseRule">基础配置</a-divider>
            <DragForm v-show="showBaseRule" v-model:api="baseForm.api"
                    :rule="baseForm.rule"
                    :option="baseForm.options"
                    @change="baseChange"></DragForm>
                    <a-divider v-show="showBaseRule">属性配置</a-divider>
        ...
        <a-divider v-show="showBaseRule">验证规则</a-divider>
        <DragForm v-show="showBaseRule" v-model:api="validateForm.api"
          ...
          <a-tab-pane key="2" name="form" tab="表单配置" style="overflow: auto">
            <DragForm :rule="form.rule" :option="form.option"
                                      v-model="form.value.form"></DragForm>
          </a-tab-pane>
      </a-tabs>
    
      <a-modal v-model:visible="preview.state" :zIndex="1020" title="预览" width="80%">
                ...
                if (data.activeRule && fapi[data.activeRule._id] === data.activeRule) {
                    if (field.indexOf('formCreate') === 0) {
                        ...
                        if (field.indexOf('effect') === 0 && field.indexOf('>') > -1) {
                            ...
                            data.activeRule['inject'] = true;//事件注入
                        } else {
                            data.activeRule[field] = value;
                        }
                    } else {
